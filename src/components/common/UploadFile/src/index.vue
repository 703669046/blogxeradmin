<template>
    <div class="upload" :class="isDropAims === true && isDrag?'Drop':''" >
        ...
        <div class="circle" v-show="logding">
            <div class="circle_c">
                <Circleview :percentage="percentage"></Circleview>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async upload(file,params){
            this.logding = true;
            let fileformData = this.getFileParam(file,params);
            try {
            ...
            return new Promise( (resolve,reject) =>{
                reader.onload = e => {
                    ...
                    img.onload = e => {
                        const w = img.width;
                        const h = img.height;
                        const quality = 0.15;  // 默认图片质量为0.92
                        // 生成canvas
